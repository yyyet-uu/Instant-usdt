...
async function completeAd(userId, sessionId, mission, provider, clickConfirmed) {
  ...
  if (adProvider === "adsbitvex" && clickConfirmed !== true) throw new Error("AdsBitvex ad was not confirmed as clicked.");
  ...
  if (!created || Date.now() - created < 12000) throw new Error("Please complete the full ad before returning.");
  ...
  await db().runTransaction(async transaction => {
    ...
    const patch = {};
    if (mission === 3) {
      const count = Math.min(50, Number(data.ad3 || 0) + 1);
      patch.ad3 = count;
      if (count >= 50) patch.mission3 = true;
...
module.exports = async function(req, res) {
  try {
    ...
    if (action === "completeAd") {
      const result = await completeAd(user.id, String(body.session || ""), Number(body.mission), String(body.provider || ""), body.clickConfirmed === true);
