...
async function stateFor(uid) {
  ...
  // When the 24-hour cooldown has expired, start a completely fresh cycle.
  // The payout cycle itself is locked immediately after withdrawal.
  const cooldown = data.cooldownUntil && data.cooldownUntil.toMillis
    ? data.cooldownUntil.toMillis()
    : Number(data.cooldownUntil || 0);
  if (snap.exists && data.withdrawn && cooldown && cooldown <= Date.now()) {
    await ref.update({
      withdrawn: false,
      cooldownUntil: null,
      mission1: false,
...
async function sendPaymentProof(uid, wallet, txHash) {
  const channel = process.env.PAYMENT_PROOF_CHANNEL || "@phone_teach";
  const url = `https://bscscan.com/tx/${txHash}`;
  const text = [
    "💸 <b>PAYMENT PROOF — INSTANT USDT</b>",
    "",
    `👤 User ID: <code>${String(uid)}</code>`,
    `💰 Amount: <b>0.10 USDT</b>`,
    "🌐 Network: <b>BSC / BEP-20</b>",
    `📥 Recipient: <code>${wallet}</code>`,
    `🔗 Transaction: <a href="${url}">View on BscScan</a>`,
    "",
...
async function payout(uid, wallet) {
  ...
  // Only lock the box and start the 24-hour cooldown after the BSC transaction
  // is actually confirmed. If the payout fails, the reward remains available.
  let receipt;
  try {
    receipt = await tx.wait();
  } catch (payoutError) {
    console.error("Payout transaction failed:", payoutError?.message || payoutError);
    throw new Error("Payout transaction failed. Your USDT Box was not locked. Please try again.");
  }
  if (!receipt || receipt.status !== 1) {
    throw new Error("Payout transaction was not confirmed successfully. Please try again.");
  }

  const cooldownUntil = new Date(Date.now() + 24 * 60 * 60 * 1000);
  await ref.update({
    // Successful payout: immediately lock the current box, clear all four keys,
    // and start a fresh 24-hour cooldown cycle.
    withdrawn: true,
    payoutWallet: wallet,
    payoutTx: tx.hash,
    payoutStatus: "paid",
  ...
  try {
    await sendPaymentProof(uid, wallet, tx.hash);
  } catch (proofError) {
    // Payment is already complete; a proof-channel failure must never undo the payout.
    console.error("Payment proof send failed:", proofError?.message || proofError);
  }

  return { tx: tx.hash, cooldownUntil: cooldownUntil.getTime() };
}
...
module.exports = async function(req, res) {
  try {
    ...
    if (action === "withdraw") {
      const result = await payout(user.id, String(body.wallet || "").trim());
      return sendJson(res, 200, {
        ok: true,
        message: `Payout sent successfully. Transaction: ${result.tx}`,
        tx: result.tx,
        cooldownUntil: result.cooldownUntil,
        state: await stateFor(user.id)
      });
    }
