...
async function completeMission1(userId) {
  ...
  return { completed: true, message: "Mission One completed.", state: await stateFor(userId) };
}

async function startAd(userId, mission) {
  ...
  const current = mission === 3 ? state.ad3 : state.ad4;
  if (current >= 50) throw new Error("This mission is already complete.");
...
async function completeAd(userId, sessionId, mission) {
  ...
  if (String(session.uid) !== String(userId) || Number(session.mission) !== mission || session.used) {
    throw new Error("Ad session is invalid or already used.");
  }

  const created = session.createdAt && session.createdAt.toMillis ? session.createdAt.toMillis() : 0;
  if (!created || Date.now() - created < 12000) {
    throw new Error("Please complete the full ad before returning.");
  }
  ...
  const field = mission === 3 ? "ad3" : "ad4";

  await db().runTransaction(async transaction => {
    const snapUser = await transaction.get(user);
    const data = snapUser.data() || {};
    const count = Math.min(50, Number(data[field] || 0) + 1);
    const patch = { [field]: count };
    if (count >= 50) patch[`mission${mission}`] = true;
    transaction.update(user, patch);
  });

  const state = await stateFor(userId);
  return {
    count: mission === 3 ? state.ad3 : state.ad4,
    completed: !!state[`mission${mission}`],
    state
  };
}
...
async function spin(uid) {
  ...
  await db().runTransaction(async transaction => {
    ...
    if (!complete) throw new Error("Complete all missions first.");
    if (data.spun) throw new Error("Mystery Box already opened.");
...
async function payout(uid, wallet) {
  ...
  if (!(state.mission1 && state.mission2 && state.mission3 && state.mission4)) {
    throw new Error("Complete all missions first.");
  }
  if (!state.spun) throw new Error("Open the Mystery Box first.");

  const ref = userRef(uid);
  const current = await ref.get();
  const data = current.data() || {};
  if (data.withdrawn) throw new Error("This reward has already been withdrawn.");
...
module.exports = async function(req, res) {
  try {
    ...
    const userRefDoc = await ensureUser(user, params.get("start_param") || "");
    ...
    if (action === "startAd") {
      return sendJson(res, 200, { ok: true, session: await startAd(user.id, Number(body.mission)) });
    }

    if (action === "completeAd") {
      const result = await completeAd(user.id, String(body.session || ""), Number(body.mission));
