...
async function startAd(userId, mission, provider) {
  ...
  if (mission === 4 && adProvider !== "monetag") {
    throw new Error("Mission Four uses Monetag only.");
  }

  let current = 0;
  if (mission === 3) current = adProvider === "adsbitvex" ? state.ad3AdsBitvex : state.ad3;
  else current = state.ad4Monetag;
  ...
  if (mission === 4 && current >= 30) throw new Error("All 30 Monetag ads are complete.");
...
async function completeAd(userId, sessionId, mission, provider, clickConfirmed = false) {
  ...
  const adProvider = String(provider || "").toLowerCase();
  if (mission === 3 && !["monetag", "adsbitvex"].includes(adProvider)) throw new Error("Mission Three uses Monetag and AdsBitvex.");
  if (mission === 4 && adProvider !== "monetag") throw new Error("Mission Four uses Monetag only.");
  ...
  await db().runTransaction(async transaction => {
    ...
    if (mission === 3) {
      ...
      const count = Math.min(30, Number(data.ad4Monetag || 0) + 1);
      patch.ad4Monetag = count;
      patch.ad4 = count;
      if (count >= 30) patch.mission4 = true;
    }
