...
async function completeAd(userId, sessionId, mission, provider, clickConfirmed = false) {
  ...
  if ((mission === 3 || mission === 4) && adProvider === "adsbitvex" && clickConfirmed !== true) throw new Error("Confirm that you clicked the advertiser button inside the AdsBitvex ad.");
...
async function payout(uid, wallet) {
  ...
  try {
    ...
    const proofChannel = process.env.PAYMENT_PROOF_CHANNEL || "@phone_teach";
    const displayName = [state.firstName, state.username ? `@${state.username}` : ""]
      .filter(Boolean)
      .join(" ") || `User ${uid}`;
    const proofText =
      `💸 PAYMENT PROOF\n\n` +
    ...
    try {
      await telegramCall("sendMessage", {
        chat_id: proofChannel,
        text: proofText,
        disable_web_page_preview: true
      });
    } catch (proofError) {
      console.error("Payment proof channel notification failed:", proofError.message || proofError);
    }
  } catch (error) {
    console.error("Payout confirmation wait failed:", error.message || error);
  }
